...
def data_save(dict_key, collages_dict, scores):
    ...
    for vv in dict_key:
        ...
        # SQL语句，写入数据表数据
        # sql = f"""
        #     select * from comp where collage like (collage)
        # """
...
def data_analysis(collages_dict):
    ...
    for i in range(0, len(dict_keys)):
        now_collage = dict_keys[i]

        if "特色汇总" in now_collage:
            continue

        scores = 0
        collages.append(now_collage)

        for now_collage_keyword in range(0, NUMBER):
            a = collages_dict[now_collage][now_collage_keyword][0]
            cnt = 0
            ...
            for j in range(0, len(dict_keys)):
                ...
                comp_collage = dict_keys[j]

                if "特色汇总" in comp_collage:
                    continue

                for comp_collage_keyword in range(0, NUMBER):
                    b = collages_dict[comp_collage][comp_collage_keyword][0]
                    # print("**************************************", a, b)
                    if a == b:
                        cnt = cnt + 1
                        # print("---------------------------------------------", a, b)
                        break
            print(scores)
            scores = scores + (1.0 - cnt / (len(dict_keys) - 1))
    ...
    #     all_keyword_sum = all_keyword_sum + all[j][1]
    ...
    #         cut = 0.0
    #         for j in range(0, len(all)):
    #             print(collages_dict[collage][i][0],all[j][0])
    #             if collages_dict[collage][i][0] == all[j][0]:
    #                 cut = collages_dict[collage][i][1] / (all[j][1]/NUMBER)
    #                 # print(cut)
    #                 break
    #         print(1-cut,sum)
    #         sum = sum + (1.0 - cut)
    ...
    # 此处调用存入数据库函数
    data_save(dict_keys, collages_dict, scores_dict)

    return collages, scores_dict
...
def line_chart(collages, cnts):
    names = collages
    highs = cnts

    line = Line("河南本科教学评估分数折线图", '2016年度', width=1200, height=600)
    line.add("分数", names, highs, mark_point=['max'], is_datazoom_show=True, is_fill=None, line_opacity=0.520,
             area_opacity=0.13)

    line.render('./datas/charts/wenqin_line.html')
